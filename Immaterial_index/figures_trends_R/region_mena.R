...
plot_trends <- function(df_decade, df_indi, region_name, min_time, max_time, span, log, name, time_size = 15) {
    ...
    myplot <- ggplot(df_decade, aes(x = decade, y = score, color = region_name)) +
        ...
        geom_vline(xintercept = c(-600, 622), linetype = "dashed", color = "grey80") +
        geom_vline(xintercept = c(622, 1800), linetype = "dashed", color = "grey80") +
        geom_text(aes(x = 11, y = text_position, size = 7), label = "Pre-Islamic period", color = "grey80", angle = 90, vjust = 0.5, hjust = 0.5) +
        geom_text(aes(x = 1211, y = text_position, size = 7), label = "Islamic period", color = "grey80", angle = 90, vjust = 0.5, hjust = 0.5)
    return(myplot)
}
