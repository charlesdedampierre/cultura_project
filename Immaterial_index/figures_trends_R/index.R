...
plot_trends <- function(df_decade, df_indi, region_name, min_time, max_time, span) {
  ...
  myplot <- ggplot(df_decade, aes(x = decade, y = score, color = region_name)) +
    ...
    geom_smooth(method = "loess", span = span, level = 0.3, linewidth = 1.5, colour = "darkblue") +
    ...
    geom_point(data = df_indi, aes(x = decade, y = score), alpha = 0.2, size = 0.5, colour = "red") +
    geom_text_repel(data = head(df_indi[order(-df_indi$score), ], 30), aes(x = decade, y = score, label = individual_name), size = 3, alpha = 5, max.overlaps = 50, color = "red") +
    theme_classic()
