...
plot_trends_duo <- function(df_decade, df_indi, min_time, max_time, span, log, name, time_size = 15) {
    ...
    if (log == "True") {
        ...
        y_axis <- "Log Immaterial Index (number of scientists)"
        y_axis_2 <- "Individual Immaterial Index (number of references in catalogs)"
    } else {
        y_axis <- "Immaterial Index (number of scientists)"
        y_axis_2 <- "Individual Immaterial Index (number of references in catalogs)"
    }
    ...
    myplot <- ggplot(df_decade, aes(x = decade, y = score, color = region_name, fill = region_name)) +
        ...
        geom_text_repel(data = head(df_indi[order(-df_indi$score), ], 20), aes(x = decade, y = score / coeff_y_axis, label = individual_name), size = 3, alpha = 5, max.overlaps = 20) +
